...
IUSE="nls static-libs elibc_FreeBSD elibc_musl"
...
src_prepare() {
	...
	if use elibc_musl; then
		sed -i 's:LIBINTL=.*$:LIBINTL=:g' configure || die "Remove intl refs"
		sed -i 's:intl::g' Makefile.in || die
	fi
}
...
src_configure() {
	...
	econf \
		...
		--disable-fsck \
		--disable-uuidd
	if [[ ((${CHOST} != *-uclibc) && (${CHOST} != *-musl)) ]] && grep -qs 'USE_INCLUDED_LIBINTL.*yes' config.{log,status} ; then
