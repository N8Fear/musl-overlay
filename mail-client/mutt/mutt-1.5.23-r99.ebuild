...
IUSE="berkdb crypt debug doc gdbm gnutls gpg idn imap kerberos mbox nls nntp pop qdbm sasl selinux sidebar slang smime smtp ssl tokyocabinet elibc_musl"
SLOT="0"
LICENSE="GPL-2"
...
src_prepare() {
	...
	if use elibc_musl; then
		sed -i -e 's:INTLLIBS=".*intl/libintl.a:INTLLIBS=":g' configure || die
		sed -i -e '/SUBDIRS =/s/intl//' Makefile.in || die
	fi
}
